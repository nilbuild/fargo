...
final class SoundBoard {
    ...
    private func soundsDirectory() -> URL {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Fargo/Sounds")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }

    private func loadSounds() {
        if let data = UserDefaults.standard.data(forKey: "fargo.soundboard"),
    ...
    private func saveSounds() {
        ...
        UserDefaults.standard.set(data, forKey: "fargo.soundboard")
    }
}
