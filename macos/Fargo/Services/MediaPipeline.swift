...
final class MediaPipeline {
    ...
    private let captureQueue = DispatchQueue(label: "com.fargo.capture", qos: .userInteractive)
    private let audioQueue = DispatchQueue(label: "com.fargo.audio", qos: .userInteractive)
    ...
    private let sessionQueue = DispatchQueue(label: "com.fargo.cameraSession", qos: .userInitiated)
    ...
    func armMicFirstFrameWatchdog() {
        ...
        micWatchdogTask = Task { [weak self] in
            ...
            print("[Fargo] Mic watchdog: no audio in 2.5s, re-attaching input")
    ...
    func armCameraFirstFrameWatchdog() {
        ...
        cameraWatchdogTask = Task { [weak self] in
            ...
            print("[Fargo] Camera watchdog: no frames in 2.5s, re-attaching input")
            self.sessionQueue.async { [self] in
                self.captureSession.beginConfiguration()
                self.addCameraInput()
                self.captureSession.commitConfiguration()
    ...
    private func armExtraCameraFirstFrameWatchdog(deviceId: String) {
        ...
        extraCameraWatchdogTasks[deviceId] = Task { [weak self] in
            ...
            print("[Fargo] Extra camera watchdog: no frames in 2.5s for \(deviceId), re-attaching input")
    ...
    private func addMicInput() {
        ...
        if let lastError {
            if lastError.lowercased().contains("not authorized") {
                micError = "Microphone access denied. Open System Settings → Privacy & Security → Microphone and enable Fargo, then relaunch the app."
            } else {
                micError = lastError
            }
        } else {
    ...
    private func applyNoiseSuppression() {
        ...
        print("[Fargo] Noise suppression \(noiseSuppression ? "enabled" : "disabled")")
    }
    ...
    private func startExtraScreenStream(spec: String) async {
        ...
        do {
            ...
            try stream.addStreamOutput(output, type: .screen,
                sampleHandlerQueue: DispatchQueue(label: "com.fargo.screen.\(spec)", qos: .userInteractive))
            try await stream.startCapture()
            extraScreenStreams[spec] = stream
            extraScreenOutputs[spec] = output
        } catch {
            print("[Fargo] Extra screen capture error for \(spec): \(error)")
        }
    }
    ...
    func updateCanvasCameraCaptures() async {
        ...
        for deviceId in activeNeededDeviceIds where extraCameraSessions[deviceId] == nil {
            guard let device = cameras.first(where: { $0.id == deviceId })?.device else {
                print("[Fargo] Extra camera: no discovered device matches saved id \(deviceId) - source will fall back to main camera texture")
                continue
            }
            let result: (AVCaptureSession, CanvasCameraCaptureDelegate)? = await withCheckedContinuation { cont in
                sessionQueue.async {
                    ...
                    do {
                        ...
                        session.commitConfiguration()
                        print("[Fargo] Extra camera error for \(deviceId): \(error)")
    ...
    private func startSystemAudioCapture() async {
        ...
        do {
            ...
            let stream = SCStream(filter: filter, configuration: config, delegate: nil)
            let sysAudioQueue = DispatchQueue(label: "com.fargo.systemaudio", qos: .userInteractive)
            ...
            print("[Fargo] System audio capture error: \(error)")
        }
    }
    ...
    func discoverScreenSources() async {
        ...
        } catch { print("[Fargo] Screen source error: \(error)") }
    }
    ...
    private func startScreenCapture() async {
        ...
        do {
            let stream = SCStream(filter: filter, configuration: config, delegate: nil)
            try stream.addStreamOutput(output, type: .screen,
                sampleHandlerQueue: DispatchQueue(label: "com.fargo.screen", qos: .userInteractive))
            try await stream.startCapture()
            screenStream = stream; isScreenCapturing = true
        } catch { print("[Fargo] Screen capture error: \(error)") }
    }
    ...
    private func stopScreenCapture() async {
        ...
        if let stream, let output {
            do {
                ...
                print("[Fargo] removeStreamOutput failed: \(error)")
            }
        }
        do {
            try await stream?.stopCapture()
        } catch {
            print("[Fargo] stopCapture failed: \(error)")
        }
    }
    ...
    func testConnection(_ dest: StreamDestination) async -> (success: Bool, message: String) {
        ...
        return await withCheckedContinuation { cont in
            ...
            conn.start(queue: DispatchQueue(label: "com.fargo.test"))
